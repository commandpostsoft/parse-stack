...
- **NEW**: `Parse::MongoDB.aggregate` and `Parse::MongoDB.find` accept a `max_time_ms:` keyword that is plumbed down to the MongoDB driver's `maxTimeMS` option. When the database cancels a query that exceeds the budget, the driver raises `Mongo::Error::OperationFailure` with code 50; parse-stack translates this into a new `Parse::MongoDB::ExecutionTimeout` exception carrying `collection_name` and `max_time_ms` attributes. `Parse::Agent::Tools.max_time_ms_for(tool_name)` derives the budget from `TOOL_TIMEOUTS` minus a 5-second buffer, so the database cancels the query before Ruby's outer `with_timeout` fires — eliminating the previous risk that `Timeout.timeout` raising into a native I/O call would corrupt the HTTP connection pool. `Parse::Agent#execute` rescues `Parse::MongoDB::ExecutionTimeout` and returns `error_code: :timeout` with a "narrow the filter, add an index, or call explain_query" suggestion. (`lib/parse/mongodb.rb`, `lib/parse/query.rb`, `lib/parse/agent/tools.rb`, `lib/parse/agent.rb`)
...
- Existing `MCPServer.run(...)` callers see no behavioral change. The WEBrick server, `/mcp` endpoint, `/health` endpoint, and `X-MCP-API-Key` authentication all continue to work as before.
...
- `Parse::Agent::MCPServer::STATIC_PROMPTS` was an internal constant and has been moved to `Parse::Agent::Prompts::BUILTIN_PROMPTS`. The `MCPServer::PROTOCOL_VERSION`, `MAX_BODY_SIZE`, `MAX_JSON_NESTING`, and `MCP_API_KEY_HEADER` constants are preserved.
